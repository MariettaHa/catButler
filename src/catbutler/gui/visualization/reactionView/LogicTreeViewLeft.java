...
public class LogicTreeViewLeft extends Group {
    ...
    LogicTreeViewLeft(ReactionTableEntry reactionTableEntry, double[] anchor, HashMap<String, Color> speciesColorMap, ArrayList<String> idxSpecies) {
    ...
    private void init() {
        String dnf = this.reactionEntry.getReactantsDNF();
        if (dnf.length() > 0 && !dnf.isBlank()) {
            if (dnf.contains(",")) {
                ...
                double[] firstPosAngle = Misc.getAnglesLeft(Misc.getAngleSpan(size), Misc.getMiddle(size), size, pathsLength, anchor[0] - 15.0, anchor[1]);
                for (String s : splitByOr) {
                    double[] posAngle = Misc.getAnglesLeft(Misc.getAngleSpan(size), splitByOr.indexOf(s), size, pathsLength, anchor[0] - 15.0, anchor[1]);
                    if (s.contains("&")) {
                        ...
                        Path path = ViewStyler.getDefaultReactionViewPath(collapsibleCircle.getCenterX(), collapsibleCircle.getCenterY(),
                                collapsibleCircle2.getCenterX(), collapsibleCircle2.getCenterY(), ViewStyler.defaultReactantsColor);
                        ...
                        double[] firstPosAngle2 = Misc.getAnglesLeft(Misc.getAngleSpan(size2), Misc.getMiddle(size2), size2, pathsLength, firstPosAngle[0], posAngle[1]);//,
                        for (String t : OrsSplitByAnd) {
                            double[] posAngle2 = Misc.getAnglesLeft(Misc.getAngleSpan(size2), OrsSplitByAnd.indexOf(t), size2, pathsLength, posAngle[0], firstPosAngle2[1]);//,
                            String str = "nan";
                            if (idxSpecies.contains(t.strip())) {
                                str = String.valueOf(idxSpecies.indexOf(t.strip()));
                            }
                            Color fill = ViewStyler.defaultReactantsColor;
                            if (speciesColorMap.containsKey(t.strip())) {
                                fill = speciesColorMap.get(t.strip());
                            }
                            CollapsibleCircle collapsibleCircle3 = new CollapsibleCircle(11.0, str, firstPosAngle2[0], posAngle2[1], fill, ViewStyler.defaultReactantsColor);

                            Path path2 = ViewStyler.getDefaultReactionViewPath(collapsibleCircle2.getCenterX(), collapsibleCircle2.getCenterY(),
                                    collapsibleCircle3.getCenterX(), collapsibleCircle3.getCenterY(), ViewStyler.defaultReactantsColor);
                        ...
                        CollapsibleCircle collapsibleCircle2 = new CollapsibleCircle(11.0, str, firstPosAngle[0], posAngle[1], fill, ViewStyler.defaultReactantsColor);
                        Path path = ViewStyler.getDefaultReactionViewPath(collapsibleCircle.getCenterX(), collapsibleCircle.getCenterY(),
                                collapsibleCircle2.getCenterX(), collapsibleCircle2.getCenterY(), ViewStyler.defaultReactantsColor);
                        this.getChildren().add(path);
                        collapsibleCircle.getChildren().add(path);
                ...
                for (String t : splitByAnd) {
                    double[] posAngle = Misc.getAnglesLeft(Misc.getAngleSpan(size), splitByAnd.indexOf(t), size, pathsLength, anchor[0] - 15.0, anchor[1]);
                    String str = "nan";
                    if (idxSpecies.contains(t.strip())) {
                        str = String.valueOf(idxSpecies.indexOf(t.strip()));
                    }
                ...
                CollapsibleCircle collapsibleCircle = new CollapsibleCircle(11.0, str, anchor[0] - 15.0, anchor[1], fill, ViewStyler.defaultReactantsColor);
                this.getChildren().addAll(collapsibleCircle, collapsibleCircle.getLabel());
            }
        } else {
